...
echo "▶ Installing Technical Debt package..."
...
if [ -z "$PROJECT_ID" ]; then
  echo "⚠️ Project may already exist, fetching ID..."
  PROJECT_ID=$(gh api graphql -f query='
...
if [ -z "$FIELD_ID" ]; then
  echo "⚠️ Status field may already exist, fetching ID..."
  FIELD_ID=$(gh api graphql -f query='
query($project:ID!) {
  node(id:$project) {
    ... on ProjectV2 {
      fields(first:100) { nodes { id name } }
    }
  }
...
echo "✅ Technical Debt package fully installed!"